...
class ScaleMinSideToSize(object):
    ...
    def __call__(self, sample):
        ...
        if 'landmarks' in sample:
            landmarks = torch.as_tensor(sample['landmarks'].reshape(-1, 2)).float()
...
class TransformByKeys(object):
    def __init__(self, transform, names, album=False):
        ...
        if self.album:
            ...
            for name in self.names:
                if name == 'image':
                    ...
                    sample[name] = np.array(transform['keypoints'], dtype='float32').reshape(-1)
        else:
...
def create_submission(path_to_data, test_predictions, path_to_submission_file):
    ...
    for i, row in mapping.iterrows():
        ...
        needed_points = points_for_image[point_index_list]
